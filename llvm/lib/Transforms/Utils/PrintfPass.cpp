...
PreservedAnalyses PrintfPass::run(Module &M, ModuleAnalysisManager &AM) {
  ...
  for (auto &F : M) {
    ...
    if (F.getName() == "main") {
      ...
      llvm::Value *openmode = builder.CreateGlobalString("r");
      auto fopen_call_instr =
          builder.CreateCall(fopen_funct, {filename, openmode});
      builder.CreateStore(global_file_descriptor, fopen_call_instr);
      ...
      for (auto &Instr : BB) {
        if (auto *CB = dyn_cast<CallBase>(&Instr)) {
          ...
          if (calledFunc->getName() == "printf") {
            ...
            auto file_desc = builder.CreateLoad(PointerTy,global_file_descriptor);
            arg_vec.push_back(file_desc);
            for (auto arg = CB->arg_begin();
                 arg != CB->arg_end(); ++arg) {
